...
class GameState():
    ...
    def getPawnMoves(self, r, c, moves): # All possible piece moves based on where the piece is
        if self.whiteToMove:
            if self.board[r-1][c] == "--": # move one square
                moves.append(Move((r, c), (r-1, c), self.board))
                if r == 6 and self.board[r-2][c] == "--": # move two squares
                    moves.append(Move((r, c), (r-2, c), self.board))
            if c-1 >= 0:
                if self.board[r-1][c-1][0] == "b": #black piece on capture square
        ...
        # black pawn moves
            ...
            if self.board[r+1][c] == "--": # move one square
                moves.append(Move((r, c), (r+1, c), self.board))
                if r == 1 and self.board[r+2][c] == "--": # move two squares
                    moves.append(Move((r, c), (r+2, c), self.board))
            if c-1 >= 0:
                if self.board[r+1][c-1][0] == "w": #white piece on capture square
                    moves.append(Move((r,c), (r+1, c-1), self.board))
                elif (r+1, c-1) == self.enPassantPossible:
                    moves.append(Move((r,c), (r+1,c-1), self.board, enPassant=True))
            if c+1 <= 7:
    ...
    def getRookMoves(self, r, c, moves):
        directions = ((-1, 0), (0, -1), (1, 0), (1, 0), (0,1))
